...
<style>
  ...
  .um-topbar { display: flex; flex-direction: column; gap: 14px; margin-bottom: 18px; }
  .um-brandTop { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; }
  .um-brandTitle {
    font-weight: 950; letter-spacing: -0.03em; font-size: 1.125rem;
    line-height: 1.1; color: var(--um-fg);
  }
  .um-brandTag {
    display: inline-flex; align-items: center; gap: 8px; padding: 5px 10px;
    border-radius: 999px; border: 1px solid var(--um-border);
  ...
  .um-brandSub { color: var(--um-muted); font-size: 13px; max-width: 54ch; margin: 0; line-height: 1.45; }
  .um-brandStack { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 6px; }
  .um-stackPill {
  ...
  .um-sessionRow { display: flex; justify-content: flex-end; margin: 4px 0 14px; }
  .um-sessionPill {
  ...
  .um-sessionPill code {
    ...
    border: 1px solid var(--um-border); border-radius: 6px; padding: 1px 6px; font-size: 12px;
  }

  .um-cardHint { color: var(--um-muted); font-size: 13px; margin: 0 0 8px; line-height: 1.45; }
  .um-kvGrid {
    display: grid; grid-template-columns: 160px 1fr; gap: 10px 14px;
    align-items: start; margin-top: 10px;
  }
  @media (max-width: 700px) { .um-kvGrid { grid-template-columns: 1fr; } }
  .um-kvKey {
    color: var(--um-muted); font-size: 12px; font-weight: 750;
    text-transform: uppercase; letter-spacing: 0.02em;
  }
  .um-kvVal { min-width: 0; font-size: 14px; }
  .um-kvVal code {
    background: color-mix(in srgb, var(--um-panel-strong) 92%, transparent);
    border: 1px solid var(--um-border); border-radius: 6px; padding: 2px 6px; font-size: 13px;
  }
