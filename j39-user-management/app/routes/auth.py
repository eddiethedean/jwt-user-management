...
async def register_submit(
    ...
    if not (email_n.endswith("@socom.mil") or email_n.endswith("@soc.mil")):
        error_msg = "Only @socom.mil and @soc.mil email addresses are permitted."
        if wants_json:
            return JSONResponse({"ok": False, "error": error_msg}, status_code=400)
        return templates.TemplateResponse(
            request,
            "register.html",
            {"request": request, "error": error_msg, "base_path": bp},
            status_code=400,
        )
    ...
    # Conditional LDAP Lookup
    if settings.directory_lookup_url:
        rec = None
        # ONLY attempt LDAP if it is a socom.mil address
        if email_n.endswith("@socom.mil"):
            try:
                rec = lookup_email(email_n)
            except Exception:
                rec = None

            # LDAP is required, reject socom.mil users who aren't found
            if settings.directory_lookup_required and not rec:
                if wants_json:
                    return JSONResponse(
                        {"ok": False, "error": "Email not found in SOCOM directory"},
                        status_code=400,
                    )
                return templates.TemplateResponse(
                    request,
                    "register.html",
                    {"request": request, "error": "Email not found in SOCOM directory.", "base_path": bp},
                    status_code=400,
                )
    try:
