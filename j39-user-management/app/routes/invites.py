...
async def accept_invite_page(
    ...
    if settings.directory_lookup_url:
        try:
            # Query the directory for the invited email
...
async def _accept(
    *, db: AsyncSession, token: str, password: str, full_name: str | None = None,
    country: str | None = None, command: str | None = None
) -> None:
    if len(password) < 8:
    ...
    if user:
        ...
        # If new user creation, use the values passed from the form
        user = User(
            email=invite.email,
            full_name=fn,
            country=c,
...
async def accept_invite_form(
    ...
    bp = base_path(request)
    try:
        await _accept(db=db, token=token, password=password, full_name=full_name, country=country, command=command)
...
async def accept_invite_api(
    ...
    fn_s = None if full_name is None else str(full_name)
    c_s = None if country is None else str(country)
    cmd_s = None if command is None else str(command)
    if not token or not password:
        raise HTTPException(status_code=422, detail="token and password are required")
    await _accept(db=db, token=token, password=password, full_name=fn_s, country=c_s, command=cmd_s)
    return {"ok": True}
