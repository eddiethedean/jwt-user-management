...
def test_admin_proxy_requires_admin_jwt_when_enabled(monkeypatch):
    monkeypatch.setenv("ADMIN_UI_REQUIRE_JWT", "1")
...
def test_admin_proxy_allows_request_with_overridden_admin_dependency(monkeypatch):
    monkeypatch.setenv("ADMIN_UI_REQUIRE_JWT", "1")
    ...
    r = c.get("/admin/")
    # Auth passes; upstream isn't ready, so 502.
    assert r.status_code == 502


def test_admin_proxy_ws_missing_token_closes_4401(monkeypatch):
    monkeypatch.setenv("ADMIN_UI_REQUIRE_JWT", "1")
    ...
    with c.websocket_connect("/admin/ws") as ws:  # missing ?token=
