...
def test_invite_api_sends_email_when_smtp_configured(tmp_path, monkeypatch) -> None:
    ...
    token = create_access_token(
        ...
        extra_claims={"is_admin": True, "email": "admin@example.com"},
    )
...
def test_inactive_user_jwt_still_valid_until_exp(tmp_path) -> None:
    """Deactivated users keep API access until the JWT expires (stateless JWT model)."""
    db_url = f"sqlite:///{tmp_path / 'test.db'}"
    app = _load_wrapped_app(db_url=db_url)
    ...
    with Session(db.engine) as s:
        user = s.get(User, uid)
        assert user
        claims = access_token_extra_claims_for_user(user)
    token = create_access_token(subject=str(uid), extra_claims=claims)
    client = TestClient(app, base_url="http://testserver")
...
def test_admin_jwt_uses_claim_not_db_is_active(tmp_path) -> None:
    db_url = f"sqlite:///{tmp_path / 'test.db'}"
    app = _load_wrapped_app(db_url=db_url)
    ...
    token = create_access_token(
        subject=str(uid),
        extra_claims={"is_admin": True, "email": "adminoff@example.com"},
    )
    client = TestClient(app, base_url="http://testserver")
    h = {"Authorization": f"Bearer {token}"}
    assert (
    ...
    assert (
        client.post(
            "/invites", headers=h, json={"email": "new.invite@example.com"}
        ).status_code
        == 200
    )
...
def test_invite_email_uses_api_accept_path(tmp_path, monkeypatch) -> None:
    db_url = f"sqlite:///{tmp_path / 'test.db'}"
    app = _load_wrapped_app(db_url=db_url)
    ...
    settings.public_base_url = "http://127.0.0.1:8001"
    settings.smtp_host = "smtp.test.local"
    settings.smtp_from_email = "noreply@test.local"
    admin_id = _seed_user(
        db_engine=db.engine,
